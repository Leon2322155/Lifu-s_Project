...
# 加载数据集
file_path = 'banana_quality_set.csv'  # 请确保文件路径正确
...
for i, (X_subset, y_subset) in enumerate(zip(X_splits, y_splits)):
    ...
    ft_values = [f"banana_{i + 1}"] + list(ft_values)  # 添加数据集标识
...
# 保存包含准确率和F1分数的CSV文件
output_file_path = 'extracted_meta_features_banana_subsets_with_metrics.csv'
