...
# 加载数据集
file_path = 'Iris_sets.csv'  # 请确保文件路径正确
data = pd.read_csv(file_path)

# 准备数据
X = data.drop(columns=['Id', 'Species']).values  # 将DataFrame转换为numpy array
y = data['Species'].values  # 将Series转换为numpy array
...
print(f"每个类的样本数量: {dict(zip(unique, counts))}")

# 分割数据集为5个子集，确保每个子集包含所有类
def stratified_split(X, y, n_splits=5):
    X_splits = []
    y_splits = []
    for _ in range(n_splits):
        X_part, X, y_part, y = train_test_split(X, y, test_size=0.8, stratify=y, random_state=None)
        X_splits.append(X_part)
        y_splits.append(y_part)
    return X_splits, y_splits

X_splits, y_splits = stratified_split(X, y)
...
for i, (X_subset, y_subset) in enumerate(zip(X_splits, y_splits)):
    ...
    ft_values = [f"iris_{i + 1}"] + list(ft_values)  # 添加数据集标识
...
# 保存包含准确率和F1分数的CSV文件
output_file_path = 'extracted_meta_features_iris_subsets_with_metrics.csv'
